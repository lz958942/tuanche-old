...
function getStyle(brandId,styleId,carId,value){
	var brandId=$("#"+brandId).val();
	jQuery.ajax({
        url: '/gson/getStyle/'+brandId,               // 提交的页面
        type: "GET",      
        dataType:'json',// 设置请求类型为"POST"，默认为"GET"
        data:{
			'format':'json',
		},
        success: function(data) {
        	var datas=data.infos;
        	var str='<option value="-1">----请选择---</option><option value="0"' +(value=='0'?' selected':'')+'>全部车型</option>';
        	if(datas!=null){
        		for(var i=0;i<datas.length;i++){
        			str+='<option value='+datas[i].id+(value==datas[i].id?' selected':'')+'>'+datas[i].name+'</option>';
        		}
        	}
        	$("#"+styleId).html(str);
        }
    });
	getCar(styleId,carId,'');
}
function getCar(styleId,carId,value){
	var styleId=$("#"+styleId).val();
	jQuery.ajax({
        url: '/gson/getCar/'+styleId+"/",               // 提交的页面
        type: "GET",      
        dataType:'json',// 设置请求类型为"POST"，默认为"GET"
        data:{
			'format':'json',
        ...
        success: function(data) {
        	var datas=data.infos;
        	var str='<option value="-1">----请选择---</option><option value=0 ' +(value=='0'?' selected':'')+'>全部车款</option>';
        	if(datas!=null){
        		for(var i=0;i<datas.length;i++){
        			str+='<option value='+datas[i].id+(value==datas[i].id?' selected':'')+'>'+datas[i].name+'</option>';
        		}
...
function getCarByValue(styleId,carId,value){
	jQuery.ajax({
        url: '/gson/getCar/'+styleId+"/",               // 提交的页面
        type: "GET",      
        dataType:'json',// 设置请求类型为"POST"，默认为"GET"
