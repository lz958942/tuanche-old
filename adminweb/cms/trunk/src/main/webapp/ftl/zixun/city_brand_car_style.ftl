	                	...
	                	<#if lists?exists>
	                		<ul class="clearfix">
	                			<#list lists as cstyle>
	                				<li><a  pl="pg=cityIndex&pl=left" href="http://${city.py}.${oldHost}/b${cstyle.id?c}/tuan/" target="_blank">${cstyle.name}</a></li>
	                			</#list>
	                			<li>
	                				<#if lists?size==0>
	                					开团筹备中，敬请期待！
                ...
                <dd>
                    <ul class="clearfix">	
		            	...
		            	<#list hotSearch as search>
		            		<li><a pl="pg=cityIndex&pl=left" href="http://${city.py}.${oldHost}/c${search.id?c}/tuan/" target="_blank">${search.name}</a></li>
		            	</#list>
	            		</#if>							
                    </ul>
                </dd>
				        			...
				        			<dl>
							            <dt>
							                <span class="lightblue">
							                	<a pl="pg=cityIndex&pl=leftIn" href="http://${city.py}.${oldHost}/b${cstyle.id?c}/tuan/" target="_blank">${cstyle.name}</a>
							                </span>
							            </dt>
							            <dd class="clearfix">
							               <#if brand_style[cstyle.id+'_']?exists>
										       ...
										       <#list styleList as sty>
										        	 <a pl="pg=cityIndex&pl=leftIn" href="http://${city.py}.${oldHost}/c${sty.id?c}/tuan/" target="_blank">${sty.name}</a>
										       </#list>
									       </#if>
							            </dd>
							        </dl>
