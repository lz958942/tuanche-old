			...
			<td>
				<div id="tabs" class="tabs">  
					<ul>
						...
						<li class="tabs_active" ><a href="/questionAnswer/toUpdateTwo">新建二级分类</a></li>
					</ul>
			   </div>
			</td>
		</tr>
</table>
		<form  id="kevinFrom" action="/questionAnswer/updateTwoKind" method="post">
		<table>
		<select name='parentId' id='parentId'>
		<option value=''>--请选择--</option>
		<c:forEach items="${kind}" var="kind">
		<option value='${kind.id}'<c:if test="${kind.id==oneId}">selected='selected'</c:if>>${kind.name}</option>
		</c:forEach>
		</select>
		<tr><td>子分类名称：<input type="hidden" value='${two.id}' name="id"/><input type="text" id='name' name='name' value='${two.name}'/></td></tr>
		<tr class='add' id="id_1">
		<td>关键词维度：<input type="text" class='dimension' name='dimension' value='${two.dimension}'></td>
		</tr>
		<tr>
		<td>页面关键字：<input type="text" class='dimension' name='pageKeyword' value='${two.pageKeyword}'></td>
		</tr>
		<tr>
		<td>页面描述：<input type="text" class='dimension' name='pageDesc' value='${two.pageDesc}'></td>
		</tr>
		<tr>
		<td>页面title：<input type="text" class='dimension' name='pageTitle' value='${two.pageTitle}'></td>
		</tr>
		<tr>
		<td>类别排序：<input type="text" class='dimension' name='sort' value='${two.sort}'></td>
		</tr>
