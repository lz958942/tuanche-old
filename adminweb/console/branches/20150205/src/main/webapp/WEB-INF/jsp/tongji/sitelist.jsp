...
<div id="man_zone">
	...
	<c:if test="${sidMap!=null&&sidMap.size()>0}">
	<div class="rb-con">
		<div class="over-auto">
			<table class="table table-bordered chargeTable">
				<thead>
					<tr style="background:none repeat scroll 0 0 #E8F1FD">
						<th></th>
						<c:forEach var="sid" items="${sidMap}">
							<th>${sid.key}</th>
						</c:forEach>
						<th>总计</th>
					</tr>
	          	...
	          	<tbody>
	          		...
	          		<c:set scope="request" var="style" value="${style=='background:none repeat scroll 0 0 #FFFBEC;'?'background:none repeat scroll 0 0 #E0FFFF':'background:none repeat scroll 0 0 #FFFBEC;' }" />
	          			<tr style="${style}">
	          				...
	          				<c:forEach var="sid" items="${sidMap}">
	          				<c:set scope="request" var="num" value="${num+result.value[sid.key] }" />
	          					<td>
	          						<c:out value="${result.value[sid.key]}" default="0" />
	          					</td>
	          				</c:forEach>
	          				<td>${num}</td>
	          			</tr>
