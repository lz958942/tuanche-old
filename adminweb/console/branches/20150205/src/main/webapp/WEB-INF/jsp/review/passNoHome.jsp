...
$(document).ready(function(){ 
	...
	function agaxPass(id,type){
		...
		if(type==0){
			...
			//添加精华
			agaxPassTwo(id,"/review/creamDispose?id="+id+"&type=1&sta=6");
		}
	}
...
<div style="height: 5px"></div>
		<form action="/review/passSearch?type=4" method="post" style="padding: 0 10px 0 10px; margin-top: 0px;" name="review" id="reviewForm">
			<table>
				<tr class="lh28">
					<td class="ti">评论编号：&nbsp;</td>
				...
				<table class="table_style table table-bordered">
					...
					<c:forEach items="${list}" var="list">
								<tr>
									...
									<a href="javascript:void(0)" onclick="pass(${list.id})">通过</a>|
									<c:if test="${list.isCream==0||list.isCream==null}">
									<a   id="a_${list.id }" href="javascript:void(0)"  onclick="cream(${list.id},1)">设置为精华</a>
									</c:if>
									<c:if test="${list.isCream==1}">
