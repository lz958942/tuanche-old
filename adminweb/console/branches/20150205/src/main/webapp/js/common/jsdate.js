...
var  canClickDate=0;//0日期全部能点击  1今天数据之后能点击   -1  之前的数据能点击
// 以上为 寒羽枫 2006-06-25 添加的变量

// 选择日期 → 由 寒羽枫 2006-06-25 添加
function InitSelectDate(canClickDateV,obj, strFormat, dfVal) {
	canClickDate=canClickDateV;
	SelectDate(obj, strFormat, dfVal);
}
function SelectDate(obj, strFormat, dfVal) {
	defaultValue = dfVal;
	var date = new Date();
	var by = date.getFullYear() - 10; // 最小值 → 50 年前
	var ey = date.getFullYear() + 10; // 最大值 → 50 年后
...
/**//**//**//**//**//**//**//**
							 ...
							 *            寒羽枫 修改为根据用户指定的 style 来确定；
							 */
// String.prototype.toDate = function(x, p) {
String.prototype.toDate = function(style) {
	/**//**//**//**//**//**//**//*
								 * if(x == null) x = "-"; if(p == null) p =
								 * "ymd"; var a = this.split(x); var y =
								 * parseInt(a[p.indexOf("y")]); //remember to
								 * change this next century ;)
								 * if(y.toString().length <= 2) y += 2000;
								 * if(isNaN(y)) y = new Date().getFullYear();
								 * var m = parseInt(a[p.indexOf("m")]) - 1; var
								 * d = parseInt(a[p.indexOf("d")]); if(isNaN(d))
								 * d = 1; return new Date(y, m, d);
								 */
...
Calendar.prototype.draw = function() {
	...
	// //因 <form> 不能嵌套， 2006-12-01 由寒羽枫改用 Div
	...
	mvAry[mvAry.length] = '    <table id="calendarTable" width="100%" style="position:absloute;border:0px solid #CCCCCC;background-color:#FFFFFF" border="0" cellpadding="3" cellspacing="1">';
	mvAry[mvAry.length] = '      <tr>';
	for ( var i = 0; i < 7; i++) {
	...
	obj.onclick = function() {
		calendar.dateControl.value = defaultValue;
		calendar.dateControl.style.color = "gray";
		calendar.hide();
	}
	...
	obj.onclick = function() {
		var today = new Date();
		/**//*
			 * calendar.date = today; calendar.year = today.getFullYear();
			 * calendar.month = today.getMonth(); calendar.changeSelect();
			 * calendar.bindData();
			 */// 2006-12-30 由民工.砖家注释
...
Calendar.prototype.changeSelect = function() {
	...
	/**//*
		 * for (var i= 0; i < cy.length; i++){ if (cy.options[i].value ==
		 * this.date.getFullYear()){ cy[i].selected = true; break; } } for (var
		 * i= 0; i < cm.length; i++){ if (cm.options[i].value ==
		 * this.date.getMonth()){ cm[i].selected = true; break; } }
		 */
	// 2006-12-30 由民工.砖家注释
}
...
Calendar.prototype.show = function(dateObj, popControl) {
	...
	// this.date = new
	// Date(dateObj.value.toDate(this.dateFormatStyle));//由寒羽枫修改，带入用户指定的 style
	this.date = new Date();// 2006-12-03 寒羽枫添加 →
	// 若为空则显示当前月份
	this.year = this.date.getFullYear();
	this.month = this.date.getMonth();
	this.changeSelect();
...
/**//**//**//**//**//**//**//*
							 * //设置控件显示或隐藏 Calendar.prototype.setDisplayStyle =
							 * function(tagName, style) { var tags =
							 * this.getElementsByTagName(null, tagName) for(var
							 * i = 0; i < tags.length; i++) { if
							 * (tagName.toLowerCase() == "select" &&
							 * (tags[i].name == "calendarYear" || tags[i].name ==
							 * "calendarMonth")){ continue; }
							 * //tags[i].style.visibility = style;
							 * tags[i].style.display = style; } }
							 */
// document.write('<div id="ContainerPanel" style="visibility:hidden"><div
// id="calendarPanel" style="position: absolute;visibility: hidden;z-index:
// 9999;');
document
